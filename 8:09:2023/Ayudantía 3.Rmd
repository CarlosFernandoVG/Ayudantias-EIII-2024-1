...
sin_plot1 <- tibble_sen %>% ggplot(aes(x = x, y = y)) + geom_line() 
sin_plot2 <- ggAcf(tibble_sen$y, lag.max = 30) + ggtitle(NULL)
sin_plot2
```
...
```{r}
(serie_2_store <- ts(diff(first_store$Weekly_Sales), start = c(2010, first_week_Store_1), frequency = 52))

serie_plot <- function(serie){
  (serie %>% forecast::autoplot()) / 
    ((serie %>% ggAcf() + ggtitle("ACF")) +  
       (serie %>% ggPacf() + ggtitle("PACF")))
}

serie_plot(serie_2_store)
```

```{r}
(serie_3_store <- ts(diff(first_store$Weekly_Sales, differences = 1, lag = 52), start = c(2010, first_week_Store_1), frequency = 52))
...
serie_plot(serie_3_store)
```
```{r}
forecast::Arima(serie_3_store, order = c(2,0,0))
```
```{r}
forecast::auto.arima(serie_3_store)
```
```{r}
model_final <- forecast::auto.arima(serie_first_store)
```

```{r}
Box.test(serie_first_store)
```
```{r}
forecast::checkresiduals(model_final)
```
+ Intervalos de confianza
+ Intervalos de predicción

```{r}
model_final %>% forecast::forecast(h = 104) %>% autoplot()
```

```{r}
forecast::Arima(serie_first_store, order = c(2,0,0))  %>% forecast::forecast(h = 104) %>% autoplot()
```
